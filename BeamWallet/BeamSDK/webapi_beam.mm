...
WebAPI_Beam::WebAPI_Beam(IConsentHandler& handler, IShadersManager::Ptr shaders, const std::string& version, const std::string& appid, const std::string& appname, beam::wallet::IWalletDB::Ptr walletDB,
                         beam::wallet::Wallet::Ptr wallet, WalletModel::Ptr walletModel)
    : _consentHandler(handler)
    , _appId(appid)
    , _appName(appname)
    {
        beam::wallet::ApiInitData data;
        ...
        _walletAPI = beam::wallet::IWalletApi::CreateInstance(version, *this, data);        
    }

    WebAPI_Beam::~WebAPI_Beam()
    {
       _walletModel->releaseAppsShaders(_appId);
    }
    ...
    void WebAPI_Beam::callWalletApi(const std::string& request)
    {
        ...
        //
        LOG_INFO () << "WebAPP API call: " << request;

        const auto stdreq = request;
        if (auto pres = _walletAPI->parseAPIRequest(stdreq.c_str(), stdreq.size()); pres)
        {
            if (pres->acinfo.appsAllowed)
            {
                ...
                return AnyThread_callWalletApiImp(stdreq);
            }

            LOG_INFO() << "Application requested call of the not allowed method: " << pres->acinfo.method;
            return AnyThread_sendError(stdreq, ApiError::NotAllowedError, std::string());
        }
        else
        {
    ...
    void WebAPI_Beam::AnyThread_callWalletApiImp(const std::string& request)
    {
        ...
        _walletModel->getAsync()->makeIWTCall(
