...
class AssetViewModel: NSObject {
    ...
    public func getAssetInfo(asset:BMAsset) -> [BMThreeLineItem] {
        ...
        let name = BMThreeLineItem(title: Localizable.shared.strings.small_unit_unit.uppercased(), detail: asset.nthUnitName, subDetail: "", titleColor: UIColor.white.withAlphaComponent(0.5), detailColor: UIColor.white, subDetailColor: UIColor.white, titleFont: BoldFont(size: 14), detailFont: RegularFont(size: 14), subDetailFont: RegularFont(size: 14), hasArrow: false)
        result.append(name)
        
        if asset.assetId > 0 {
            let longDesc = BMThreeLineItem(title: Localizable.shared.strings.asset_id.uppercased(), detail: "\(asset.assetId)", subDetail: "", titleColor: UIColor.white.withAlphaComponent(0.5), detailColor: UIColor.white, subDetailColor: UIColor.white, titleFont: BoldFont(size: 14), detailFont: RegularFont(size: 14), subDetailFont: RegularFont(size: 14), hasArrow: false)
            result.append(longDesc)
        }
        
        if !asset.shortDesc.isEmpty {
            let shortDesc = BMThreeLineItem(title: Localizable.shared.strings.short_desc.uppercased(), detail: asset.shortDesc, subDetail: "", titleColor: UIColor.white.withAlphaComponent(0.5), detailColor: UIColor.white, subDetailColor: UIColor.white, titleFont: BoldFont(size: 14), detailFont: RegularFont(size: 14), subDetailFont: RegularFont(size: 14), hasArrow: false)
            result.append(shortDesc)
        }

        if !asset.longDesc.isEmpty {
            let longDesc = BMThreeLineItem(title: Localizable.shared.strings.long_desc.uppercased(), detail: asset.longDesc, subDetail: "", titleColor: UIColor.white.withAlphaComponent(0.5), detailColor: UIColor.white, subDetailColor: UIColor.white, titleFont: BoldFont(size: 14), detailFont: RegularFont(size: 14), subDetailFont: RegularFont(size: 14), hasArrow: false)
            result.append(longDesc)
        }
    ...
    public func getAssetBalanceInfo(asset:BMAsset)-> [[BMThreeLineItem]] {
        ...
        let locked = BMThreeLineItem(title: Localizable.shared.strings.locked.uppercased(), detail: asset.isBeam() ? String.currency(value: lockedBalance) : String.currency(value: lockedBalance, name: asset.unitName), subDetail: ExchangeManager.shared().exchangeValueAsset(lockedBalance, assetID: asset.assetId), titleColor: UIColor.white.withAlphaComponent(0.5), detailColor: UIColor.white, subDetailColor: UIColor.white, titleFont: BoldFont(size: 14), detailFont: RegularFont(size: 14), subDetailFont: RegularFont(size: 14), hasArrow: true)
